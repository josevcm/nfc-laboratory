...
Copy-Item -Force -Recurse config   $BuildPath
Copy-Item -Force -Recurse packages $BuildPath

# Copy application resources
Copy-Item -Force -Recurse ../dat/fonts  $BuildPath/packages/nfc-lab/data
Copy-Item -Force -Recurse ../dat/config $BuildPath/packages/nfc-lab/data

# Copy application external libraries
Copy-Item -Force ../dll/glew-2.1.0/x86_64-w64-mingw32/bin/*.dll      $BuildPath/packages/nfc-lab/data/
Copy-Item -Force ../dll/usb-1.0.20/x86_64-w64-mingw32/bin/*.dll      $BuildPath/packages/nfc-lab/data/
Copy-Item -Force ../dll/freetype-2.11.0/x86_64-w64-mingw32/bin/*.dll $BuildPath/packages/nfc-lab/data/
...
# Create QT desployment from executable
...
# Create QT installer
binarycreator.exe -c $BuildPath/config/config.xml -p $BuildPath/packages $BuildPath/nfc-lab-1.0.0-x86_64.exe

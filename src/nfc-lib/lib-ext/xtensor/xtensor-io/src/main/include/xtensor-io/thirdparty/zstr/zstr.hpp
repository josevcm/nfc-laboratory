...
public:
        ...
        : is_input(_is_input)
    {
        ...
        if (is_input)
        {
            this->avail_in = 0;
            this->next_in = Z_NULL;
            ret = inflateInit2(this, -MAX_WBITS);
        }
        else
        {
            ret = deflateInit2(this, 8, Z_DEFLATED, -MAX_WBITS, _level, Z_DEFAULT_STRATEGY);
        }
...
public:
    istreambuf(std::streambuf * _sbuf_p,
               std::size_t _buff_size = default_buff_size, bool _auto_detect = false)
        : sbuf_p(_sbuf_p),
          zstrm_p(nullptr),
...
public:
    ...
    ostream(std::ostream & os, int compression_level)
        : std::ostream(new ostreambuf(os.rdbuf(), (std::size_t) 1 << 20, compression_level))
    {
        exceptions(std::ios_base::badbit);
    }

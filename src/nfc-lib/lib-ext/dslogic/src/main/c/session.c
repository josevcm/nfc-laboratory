 ...
 *         SR_ERR_MALLOC upon memory allocation errors.
 */
SR_PRIV int sr_session_source_add(int fd, int events, int timeout, sr_receive_data_callback_t cb, const struct sr_dev_inst *sdi)
{
   GPollFD p;
 ...
 *         SR_ERR_MALLOC upon memory allocation errors.
 */
SR_PRIV int sr_session_source_add_pollfd(GPollFD *pollfd, int timeout, sr_receive_data_callback_t cb, const struct sr_dev_inst *sdi)
{
   return _sr_session_source_add(pollfd, timeout, cb, sdi, (gintptr) pollfd);
}

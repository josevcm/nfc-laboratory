...
SR_PRIV int dsl_setup_probes(struct sr_dev_inst *sdi, int num_probes)
{
   ...
   {
      if (!(probe = sr_channel_new(j, channel_modes[devc->ch_mode].type, TRUE, probe_names[j])))
         return SR_ERR;
...
static void receive_transfer(struct libusb_transfer *transfer)
{
   ...
   if (devc->status == DSL_DATA && transfer->actual_length != 0)
   {
      ...
      if ((devc->limit_samples && (devc->num_bytes < devc->actual_bytes || devc->is_loop)) || sdi->mode != LOGIC)
      {
         if (!devc->is_loop)
         {
      ...
      if (sdi->mode == LOGIC && devc->limit_samples && !devc->is_loop && devc->num_bytes >= devc->actual_bytes)
      {
         devc->status = DSL_STOP;
      }
      else if ((sdi->mode == DSO && devc->instant) && devc->limit_samples && devc->num_samples >= devc->actual_samples)
      {
...
static void receive_header(struct libusb_transfer *transfer)
{
   ...
   if (!devc->abort && transfer->status == LIBUSB_TRANSFER_COMPLETED && trigger_pos->check_id == TRIG_CHECKID)
   {
      sr_info("%llu: receive_trigger_pos(): status %d; timeout %d; received %d bytes.", (u64_t) g_get_monotonic_time(), transfer->status, transfer->timeout, transfer->actual_length);
      ...
      {
         if (sdi->mode != LOGIC || devc->stream || remain_cnt < devc->limit_samples)
         {
...
SR_PRIV int dsl_start_transfers(const struct sr_dev_inst *sdi)
{
   ...
   libusb_fill_bulk_transfer(transfer, usb->devhdl, 6 | LIBUSB_ENDPOINT_IN, (unsigned char *) trigger_pos, dsl_header_size(devc), (libusb_transfer_cb_fn) receive_header, devc, 0);

   if ((ret = libusb_submit_transfer(transfer)) != 0)
   {
      sr_err("%s: Failed to submit trigger_pos transfer: %s.", __func__, libusb_error_name(ret));
      libusb_free_transfer(transfer);
      g_free(trigger_pos);
      devc->status = DSL_ERROR;
      return SR_ERR;
   ...
   for (i = 1; i <= num_transfers; i++)
   {
      ...
      libusb_fill_bulk_transfer(transfer, usb->devhdl, 6 | LIBUSB_ENDPOINT_IN, buf, size, (libusb_transfer_cb_fn) receive_transfer, devc, 0);

      if ((ret = libusb_submit_transfer(transfer)) != 0)
      {
         sr_err("%s: Failed to submit transfer: %s.", __func__, libusb_error_name(ret));
         libusb_free_transfer(transfer);
         g_free(buf);
         devc->status = DSL_ERROR;
         devc->abort = TRUE;
...
SR_PRIV int dsl_secuReset(const struct sr_dev_inst *sdi)
{
   if (dsl_wr_reg(sdi, SEC_CTRL_ADDR, 0) != SR_OK)
      goto Err;
   if (dsl_wr_reg(sdi, SEC_CTRL_ADDR + 1, 0) != SR_OK)
      goto Err;
   g_usleep(10 * 1000);
   if (dsl_wr_reg(sdi, SEC_CTRL_ADDR, 1) != SR_OK)
      goto Err;
   if (dsl_wr_reg(sdi, SEC_CTRL_ADDR + 1, 0) != SR_OK)
      goto Err;
...
SR_PRIV int dsl_secuWrite(const struct sr_dev_inst *sdi, uint16_t cmd, uint16_t din)
{
   if (dsl_wr_reg(sdi, SEC_DATA_ADDR, din) != SR_OK)
      goto Err;
   if (dsl_wr_reg(sdi, SEC_DATA_ADDR + 1, (din >> 8)) != SR_OK)
      goto Err;
   if (dsl_wr_reg(sdi, SEC_CTRL_ADDR, cmd) != SR_OK)
      goto Err;
   if (dsl_wr_reg(sdi, SEC_CTRL_ADDR + 1, (cmd >> 8)) != SR_OK)
      goto Err;
...
{
   uint16_t sec;
   if (dsl_rd_reg(sdi, SEC_DATA_ADDR + 1, (uint8_t * ) & sec) != SR_OK)
      goto Err;
   sec <<= 8;
   if (dsl_rd_reg(sdi, SEC_DATA_ADDR, (uint8_t * ) & sec) != SR_OK)
      goto Err;

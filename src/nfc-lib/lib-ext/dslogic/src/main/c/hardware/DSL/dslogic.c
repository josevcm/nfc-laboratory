...
static GSList *scan(GSList *options)
{
   ...
   for (i = 0; devlist[i]; i++)
   {
      ...
      if (dsl_check_conf_profile(device_handle))
      {
         ...
         sr_info("Found a DSLogic device,name:\"%s\",handle:%p, pid:%02x", prof->model, device_handle, prof->pid);
         ...
         sdi = sr_dev_inst_new(channel_modes[devc->ch_mode].mode, SR_ST_INITIALIZING, prof->vendor, prof->model, prof->model_version);
...
static int config_get(int id, GVariant **data, const struct sr_dev_inst *sdi, const struct sr_channel *ch, const struct sr_channel_group *cg)
{
   ...
   if (ret != SR_OK)
   {
      switch (id)
      {
         ...
         case SR_CONF_WAIT_UPLOAD:
            if (devc->buf_options == SR_BUF_UPLOAD && devc->status == DSL_START)
            {
...
static int config_set(int id, GVariant *data, struct sr_dev_inst *sdi, struct sr_channel *ch, struct sr_channel_group *cg)
{
   struct DSL_context *devc;
   const char *stropt;
   ...
   else if (id == SR_CONF_DEVICE_MODE)
   {
      ...
      if (sdi->mode == LOGIC)
      {
         ...
         for (i = 0; i < ARRAY_SIZE(channel_modes); i++)
         {
            if (channel_modes[i].mode == LOGIC && devc->profile->dev_caps.channels & (1 << i))
            {
      ...
      else if (sdi->mode == ANALOG)
      {
         ...
         for (i = 0; i < ARRAY_SIZE(channel_modes); i++)
         {
            if (channel_modes[i].mode == ANALOG && devc->profile->dev_caps.channels & (1 << i))
            {
               devc->ch_mode = channel_modes[i].id;
               num_probes = channel_modes[i].num;
               devc->stream = channel_modes[i].stream;
   ...
   else if (id == SR_CONF_OPERATION_MODE)
   {
      ...
      if (sdi->mode == LOGIC && devc->op_mode != nv)
      {
         ...
         {
            devc->cur_samplerate = devc->stream ? channel_modes[devc->ch_mode].max_samplerate / 10 : SR_MHZ(100);
            devc->limit_samples = devc->stream ? devc->cur_samplerate * 3 : devc->profile->dev_caps.hw_depth / dsl_en_ch_num(sdi);
         }
      }

...
std::string lime::locateImageResource(const std::string &name)
{
    ...
    {
        const std::string fullPath(searchPath + "/19.01/" + name);
...
{
    const std::string destDir(lime::getAppDataDirectory() + "/images/19.01");
    const std::string destFile(destDir + "/" + name);
    const std::string sourceUrl("https://downloads.myriadrf.org/project/limesuite/19.01/" + name);
